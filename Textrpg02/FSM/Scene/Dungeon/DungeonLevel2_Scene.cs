...
{
    internal class DungeonLevel2_Scene : SceneBase
    {
        ...
        private int floorNum = 2; //현재 층수
        private string dungeonName = "레나르의 호수"; //현재 던전 이름
        private string dungeonNameNext = "케리네이아의 야영지"; //다음 던전 이름
        public static bool battleResult; //전투 결과


        public DungeonLevel2_Scene(SceneController controller) : base(controller)
        {
        }
                ...
                if (Calculate())
                {
                    ...
                    switch (input)
                    {
                        ...
                        case "1":
                            ...
                            Console.WriteLine("...터벅...");
                            Sleep();
                            controller.ChangeSceneState(controller.BattleLevel3_Scene);
                            break;
                        default:
