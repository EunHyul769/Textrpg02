...
{
    internal class DungeonLevel1_Scene : SceneBase
    {
        ...
        private int floorNum = 1; //현재 층수
        private string dungeonName = "네메아 사자의 방"; //현재 던전 이름
        private string dungeonNameNext = "레나르의 호수"; //다음 던전 이름
        public static bool battleResult; //전투 결과


        public DungeonLevel1_Scene(SceneController controller) : base(controller)
        {
        }
                ...
                if (Calculate())
                {
                    ...
                    switch (input)
                    {
                        ...
                        case "1":
                            ...
                            Console.WriteLine("...터벅...");
                            Sleep();
                            controller.ChangeSceneState(controller.BattleLevel2_Scene);
                            break;
                        default:
