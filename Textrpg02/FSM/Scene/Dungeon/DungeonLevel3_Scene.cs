...
{
    internal class DungeonLevel3_Scene : SceneBase
    {
        ...
        private int floorNum = 3; //현재 층수
        private string dungeonName = "케리네이아의 야영지"; //현재 던전 이름
        private string dungeonNameNext = "에리만토스의 산림"; //다음 던전 이름
        public static bool battleResult; //전투 결과


        public DungeonLevel3_Scene(SceneController controller) : base(controller)
        {
        }
                ...
                if (Calculate())
                {
                    ...
                    switch (input)
                    {
                        ...
                        case "1":
                            ...
                            Console.WriteLine("...터벅...");
                            Sleep();
                            controller.ChangeSceneState(controller.BattleLevel4_Scene);
                            break;
                        default:
